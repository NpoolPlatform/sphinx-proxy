...
func getProxySign() *mSign {
	slk.RLock()
	defer slk.RUnlock()
	return lmSign[rnd.Intn(len(lmSign))]
}

func getProxyPlugin(coinType sphinxplugin.CoinType) *mPlugin {
	plk.RLock()
	defer plk.RUnlock()
	return lmPlugin[coinType][rnd.Intn(len(lmPlugin[coinType]))]
}
...
func (s *mSign) signStreamRecv() {
	for {
		...
		case signproxy.TransactionType_Signature:
			pluginProxy := getProxyPlugin(ssResponse.GetCoinType())
...
func ConsumerMQ() error {
	...
	for info := range consumerInfo {
		...
		case signproxy.TransactionType_TransactionNew:
			mPlugin := getProxyPlugin(tinfo.CoinType)
